...
export default function SideBar({ isMobile }: medaQuery) {
  ...
  const currentIcon = Icons.find((icon) => icon.isActive)?.icon;
  ...
  if (isMobile) {
    return (
      <Grid
        ...
        style={isChatOpen || isMobile ? { display: "none" } : {}}
      >
        {Icons.map((icon) => (
          <SideBarIconButton
            key={icon.id}
  ...
  return (
    ...
    >
      ...
      >
        {Icons.map((icon) => (
          <Grid key={icon.id}>
            <SideBarIconButton icon={icon} icons={Icons} setIcons={setIcons} />
          </Grid>
        ))}
      </Grid>
